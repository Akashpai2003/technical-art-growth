...
export class FlowerEngine {
  ...
  reset() {
    this.root = {
      ...
      angle: -Math.PI / 2, 
      length: this.canvasHeight * 0.15, // Shorter base trunk to branch earlier
      depth: 0,
      progress: 0,
      children: [],
      isFlower: false,
  ...
  preGenerate(branch: Branch) {
    ...
    let maxSplits = 2;
    if (branch.depth === 0) maxSplits = 3;
    if (branch.depth > 1 && Math.random() > 0.7) maxSplits = 1; 
    
    const splits = maxSplits;
    const spread = GROWTH_PARAMS.branchAngle + (1 - depthRatio) * 0.2;
    
    for (let i = 0; i < splits; i++) {
        // Distribute branching positions along the parent
        let pos = 1.0;
        if (splits === 1) pos = 0.7 + Math.random() * 0.3;
        else if (splits === 2) pos = 0.4 + i * 0.4 + Math.random() * 0.2;
        else if (splits === 3) pos = 0.2 + i * 0.3 + Math.random() * 0.2;
           
        const len = this.canvasHeight * 0.2 * Math.pow(0.85, branch.depth) * (0.8 + Math.random() * 0.4);
    ...
    else if (depthRatio >= 0.6) flowerChance = GROWTH_PARAMS.flowerDensity * 0.4;
  ...
  _drawBranch(ctx: CanvasRenderingContext2D, branch: Branch, startX: number, startY: number, accumulatedBend: number, theme: ThemeConfig, baseBend: number, time: number) {
    ...
    if (depthRatio > 0.3 && branch.progress > 0.1) {
       ...
       let leafBaseCount = 0;
       if (depthRatio > 0.7) leafBaseCount = 8;
       else if (depthRatio > 0.4) leafBaseCount = 4;
       else leafBaseCount = 1;
       ...
       for (let i = 0; i < leafCount; i++) {
          ...
          if (branch.progress > t) {
             ...
             const leafScaleBase = 0.8 + random() * 0.6;
             ...
             const leafW = 5 + random() * 3; 
             const leafL = 14 + random() * 8;
  ...
  _drawParticles(ctx: CanvasRenderingContext2D, theme: ThemeConfig) {
      for (const p of this.particles) {
          ...
          if (p.type === 'leaf') {
             ...
             ctx.globalAlpha = p.grounded ? 0.4 : 0.9;
             ctx.fillStyle = theme.flowers;
