...
export class FlowerEngine {
  ...
  reset() {
    // Target 90% viewport height
    this.root = {
      id: Math.random().toString(),
      angle: -Math.PI / 2, 
      length: this.canvasHeight * 0.36, 
      depth: 0,
      progress: 0,
      children: [],
      isFlower: false,
  ...
  _drawBranch(ctx: CanvasRenderingContext2D, branch: Branch, startX: number, startY: number, accumulatedBend: number, theme: ThemeConfig, baseBend: number, time: number) {
    ...
    const startWidth = Math.max(1.0, (GROWTH_PARAMS.maxDepth - branch.depth + 1) * 1.6 * branch.thicknessMult);
    const endWidth = Math.max(0.3, (GROWTH_PARAMS.maxDepth - branch.depth) * 1.6 * branch.thicknessMult);

    const segments = Math.max(4, Math.floor(currentLength / 15));
    
    const passes = [
        { mult: 3.5, alpha: 0.04, comp: 'screen' },
        { mult: 2.0, alpha: 0.10, comp: 'screen' },
        { mult: 1.2, alpha: 0.25, comp: 'source-over' },
        { mult: 0.8, alpha: 0.70, comp: 'source-over' },
        { mult: 0.4, alpha: 1.00, comp: 'source-over' },
    ];
    ...
    for (const pass of passes) {
        ...
        for (let i = 1; i <= segments; i++) {
            ...
            if (pass.mult > 1.0) {
                 wiggleX = (Math.random() - 0.5) * pass.mult * 1.5;
                 wiggleY = (Math.random() - 0.5) * pass.mult * 1.5;
            }
            ...
            ctx.lineWidth = currentWidth * pass.mult;
            ctx.globalAlpha = pass.alpha;
            // Use glowColor for outer passes for a luminous halo
            ctx.strokeStyle = pass.mult > 1.2 ? theme.glowColor : (depthRatio > 0.6 ? theme.leaves : theme.tree);
            ctx.globalCompositeOperation = pass.comp as any;
    ...
    if (depthRatio > 0.2 && branch.progress > 0.1) {
       ...
       for (let i = 0; i < leafCount; i++) {
          ...
          if (branch.progress > t) {
             ...
             // Leaf outer halo (watercolor bleed)
             ctx.beginPath();
             ctx.moveTo(0, 0);
             ctx.quadraticCurveTo(leafW * 1.4, -leafW * 0.6, leafL * 1.2, 0);
             ctx.quadraticCurveTo(leafW * 1.4, leafW * 0.6, 0, 0);
             ...
             ctx.beginPath();
             ctx.moveTo(0, 0);
             ctx.quadraticCurveTo(leafW * 1.1, -leafW * 0.5, leafL * 1.05, 0);
             ctx.quadraticCurveTo(leafW * 1.1, leafW * 0.5, 0, 0);
             ctx.globalCompositeOperation = 'source-over';
             ...
             ctx.beginPath();
             ctx.moveTo(0, 0);
             ctx.quadraticCurveTo(leafW * 0.7, -leafW * 0.3, leafL * 0.8, 0);
             ctx.quadraticCurveTo(leafW * 0.7, leafW * 0.3, 0, 0);
             ctx.fillStyle = theme.leaves;
             ctx.globalAlpha = 0.9;
             ctx.fill();
  ...
  _drawFlowers(ctx: CanvasRenderingContext2D, branch: Branch, startX: number, startY: number, accumulatedBend: number, theme: ThemeConfig, baseBend: number, time: number) {
    ...
    if (branch.isFlower && branch.flowerProgress > 0) {
      ...
      // Ethereal luminous bloom (multiple screen layers)
      ctx.globalCompositeOperation = 'screen';
      
      for (let g = 3; g >= 1; g--) {
          ctx.beginPath();
          ctx.arc(0, 0, flowerRadius * (1 + g * 0.8), 0, Math.PI * 2);
          ctx.fillStyle = theme.glowColor;
  ...
  _drawParticles(ctx: CanvasRenderingContext2D, theme: ThemeConfig) {
      for (const p of this.particles) {
          ...
          if (p.type === 'leaf') {
             ctx.globalCompositeOperation = 'source-over';
             ctx.globalAlpha = p.grounded ? 0.15 : 0.8;
             ctx.fillStyle = theme.leaves;
             ctx.beginPath();
             ctx.moveTo(0, 0);
             ...
             ctx.globalCompositeOperation = 'screen';
             ctx.globalAlpha = p.grounded ? 0.05 : 0.3;
             ctx.fillStyle = theme.glowColor;
             ctx.fill();
          } else {
             ctx.globalCompositeOperation = 'screen';
             ctx.globalAlpha = p.grounded ? 0.2 : 0.85;
             ctx.fillStyle = theme.flowers;
